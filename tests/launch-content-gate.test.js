...
test("heritage prose and app-authored interpretation never share a palace (M0 reconciliation)", () => {
  // Ten palaces carry tradition-attributed prose (DR-2026-09-09-R8-TWELVE-PALACES-RESTORED);
  // the two whose placement the project's own sources dispute carry the L-05
  // app-authored interpretation instead, so no paid palace is empty.
  assert.equal(PALACES.length, 12);
  const disputed = PALACES.filter((p) => p.heritageStatus === "WITHHELD_STRUCTURAL_DISAGREEMENT");
  assert.deepEqual(disputed.map((p) => p.key).sort(), ["property", "wealth"]);
  for (const p of PALACES) {
    if (p.heritageStatus === "RUNTIME_PROSE") {
      assert.ok(p.reading, `${p.key}: tradition palace has no reading`);
      assert.equal(p.interpretation, null, `${p.key}: app text must not sit beside heritage prose`);
      continue;
    }
    assert.ok(p.interpretation, `${p.key}: no interpretation — the paid palace would be empty`);
